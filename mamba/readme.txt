...
2) Mamba needs to reload Iris Manager self (iris_manager.self) because Cobra needs to get vsh process 
(reload causes vsh child process and i get it from here). Code is protected from vsh_process NULL condition.

2) Some functions of cobralib are disabled: Iris Manager uses minimal cobralib named 'cobre'

3) Spoof functions are disabled from 4.53: iris Manager don´t support spoof method

4) Others functions for patches can be disabled

5) Multiman is blocked to avoid problems to the users (reboot the console to use it)

6) 'Mamba' is loaded AFTER of syscall8 Iris Manager payload and uses it for example, for HTAB method.

7) 'Mamba' can be detected using the sys8_mamba() syscalls from Iris Manager: if it return 0x666 is 'Mamba' (and not Cobra)
