...
void client_thread(void *conn_s_p)
{
    ...
    while(appstate != 1 && (bytes = recv(conn_s, temp, 511, 0)) > 0)
    {
        ...
        if(strcmp2(cmd, "FEAT") == 0)
        {
            char *feat[] =
            {
                "REST STREAM", "PASV", "PORT", "MDTM", "MLSD", "SIZE", "SITE CHMOD", "SITE COPY", "SITE RESTART", "SITE SHUTDOWN", "APPE",
...
cdup:           bytes = 0;
            ...
            {
                if(!strncmp(cwd, "/ntfs", 5) || !strncmp(cwd, "/ext", 4))
                {
                    DIR_ITER *fd = NULL;
                    ...
                    if(sysLv2FsOpenDir(cwd, &fd) == 0)
                    {
                        ...
                        if(!strcmp(cwd, "/"))
                        {
                            ...
                            for(int i = 1; i < 9; i++)
                            {
                                if(mounts[i])
                                {
                                    bytes = sprintf(temp, "%s%s%s%s%s%s%s%s%s%s   1 nobody   nobody   %10llu %s ntfs%i:\r\n",
                                        fis_dir(stat) ? "d" : "-",
                                        ((stat.st_mode & S_IRUSR) != 0) ? "r" : "-",
                                        ((stat.st_mode & S_IWUSR) != 0) ? "w" : "-",
                                        ((stat.st_mode & S_IXUSR) != 0) ? "x" : "-",
            ...
            if(strcmp2(cmd, "STOR") == 0 || strcmp2(cmd, "APPE") == 0)
            {
                if(itemp == 1)
                {
                    ...
                    if(!strncmp(temp, "/ntfs", 5) || !strncmp(temp, "/ext", 4))
                    {
                        s32 fd;
                        fd = ps3ntfs_open(temp, O_WRONLY | O_CREAT | (strcmp2(cmd, "APPE") == 0 ? O_APPEND : O_TRUNC), 0);
