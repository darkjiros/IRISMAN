...
int cover_update(char *title_id)
{
...

    {
        ...
        sprintf(temp_buffer + 3072, "%s/config/ps3tdb.txt", self_path);
        int file_size = 0;
    ...
    // covers
    sprintf(temp_buffer + 1024, "%s", id);
    ret = get_icon(temp_buffer + 1024, 0);
    ...
    if(ret < 0 || !game_up_mode)
    {
        ...
        if(stat(temp_buffer, &s))
        {
            ...
            if(stat(temp_buffer, &s))
            {
                ...
                sprintf(temp_buffer + 1024, "%s%s.JPG", covers_path, id);
                mkdir_secure(temp_buffer);
            }
        }
...
no_system:
    ...
    if(max_list > 0)
    {
        ...
        sprintf(temp_buffer + 1024, updates_path);
        ...
        for(n = 0; n < max_list; n++)
        {
            ...
            sprintf(temp_buffer + 1024, "%s%s", updates_path, o);
